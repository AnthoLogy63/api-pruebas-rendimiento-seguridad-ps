/**
 * K6 — Escenario 3: 100 usuarios | Ramp-Up 30s | 15 iteraciones
 * ==============================================================
 * Uso: k6 run tests/k6/escenario3.js
 */
  ...
  group("Libros POST", () => {
    if (__VU <= 5) {
      const p = JSON.stringify({
        titulo: `Libro E3 VU${__VU} IT${__ITER}`, autor: "Autor K6",
        isbn: `978-E3-${__VU}-${Date.now()}`, editorial: "Nova",
        anio_publicacion: 2024, categoria: "Prueba", cantidad_disponible: 2,
      });
      const r = http.post(`${BASE_URL}/libros`, p, { headers });
...
export function handleSummary(data) {
  ...
  const resumen = {
    escenario: "Escenario 3 — 100 VUs | Ramp-Up 30s | 15 iteraciones",
  ...
  const txt = [
    "═".repeat(60),
    "  RESULTADO K6 — ESCENARIO 3",
    "  100 VUs | Ramp-Up 30s | 15 iteraciones por VU",
    "═".repeat(60),
  ...
  return {
    stdout: txt,
    "tests/k6/results/e3_summary.json": JSON.stringify(resumen, null, 2),
  };
}

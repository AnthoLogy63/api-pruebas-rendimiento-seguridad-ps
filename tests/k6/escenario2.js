/**
 * K6 — Escenario 2: 50 usuarios | Ramp-Up 20s | 10 iteraciones
 * ==============================================================
 * Uso: k6 run tests/k6/escenario2.js
 */
  ...
  group("Libros POST", () => {
    if (__VU <= 5) {
      const p = JSON.stringify({
        titulo: `Libro E2 VU${__VU} IT${__ITER}`, autor: "Autor K6",
        isbn: `978-E2-${__VU}-${Date.now()}`, editorial: "Nova",
...
export function handleSummary(data) {
  ...
  const resumen = {
    escenario: "Escenario 2 — 50 VUs | Ramp-Up 20s | 10 iteraciones",
  ...
  const txt = [
    "═".repeat(60),
    "  RESULTADO K6 — ESCENARIO 2",
    "  50 VUs | Ramp-Up 20s | 10 iteraciones por VU",
    "═".repeat(60),
  ...
  return {
    stdout: txt,
    "tests/k6/results/e2_summary.json": JSON.stringify(resumen, null, 2),
  };
}
